...
- 三平台统一使用更易读的字号；Windows 额外使用优化后的系统字体，并让主控制台和首次向导随宽屏或最大化窗口流式扩展
- Windows OpenSSH 操作在后台运行，不闪现额外终端窗口
...
- 可继续使用 Paldeck 受管部署、显式导入兼容的现有部署，或安全创建新的远程部署
...
- 手动保存世界时立即显示进度、保留 REST 失败结果，并根据远程存档文件时间确认
  成功请求是否真正落盘
- 停止或重启前可自定义广播和倒计时，并验证世界保存；广播或保存失败时中止停服操作
...
部署、存档或备份；删除最后一条记录后会返回初始化向导。多服务器配置管理以及备份
策略、删除和恢复操作目前标记为 **Beta**，等待更多打包客户端和远程服务器验证。
试用时请另外保留原连接信息和一份独立存档备份。
